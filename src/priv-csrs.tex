...
在RISC-V指令集中，系统主操作码编码所有特权指令。这些特权指令可以分为两类，
...
除了在本手册第一卷中描述的非特权指令集外，具体实现可能包含一些额外的控制
状态寄存器，这些控制和状态寄存器可通过本手册第一卷所描述的控制状态寄存器指令所
规定的特权级别进行访问。本章主要描述控制寄存器的地址空间，后续章节根据特权级别
描述每个状态控制寄存器的功能，以及与某个具体的特权级别相关的一些特权指令。(注：
虽然控制寄存器与指令通过特权级别关联，但是它们可以被更高的特权级别访问)
...
在具体的实现中，可能允许一个更高特权级别捕获低级别对应的信号。这些改变
对低特权级别的软件应该透明。
...
试图访问一个不存在CSR会引发一个不合法的指令异常。以不适应的特权
级别访问CSR或写一个只读寄存器也会导致合法的指令异常。一个读/写寄存器
中的某些位可能是只读的，在这种情况下，写操作针对只读的位忽略。
...
在写这些字段时，应该保留这些字段的值。为了前向兼容，具体的实现必须将这些字段
...
为了简化软件模型，在后向兼容时，CSR中的预留字段在将来的定义中，
在更新CSR其它字段时必须处理非原子的读/修改/写顺序。或者，原始的CSR定义
...
硬件实现只需实现足够的状态位区分支持的值，但是必须读操作返回时，必须
返回完整的任何支持值的指定位编码。
\end{commentary}
...
某些读/写操作CSR段只定义部分编码，在能够保证读操作返回合法值的前提下，
允许写入任何值。假设写入CSR没有副作用，支持值的范围可以通过写入预期的值
然后查看是否保留。这些字段在寄存器描述中标记为 \warl\
...
在CSR上的隐式读操作返回的值与在其上显示读操作返回的值式一致的，
...
如果CSR的宽度发生了改变（比如，通过修改在~\ref{xlen-control}描述的长度控制），
在没有特殊说明的情况下CSR中{\em writable}字段的值和具有新宽度的CSR依然由之
前的宽度确定。
\begin{enumerate}
...
\item 对于前宽度CSR中的只读位，临时寄存器中的相应位置为0
...
\item 临时寄存器的宽度根据新的宽度进行调整。如果新的宽度比之前的宽度要小，则
临时寄存器中的低位比特被保留，而高位比特则被丢弃。如果新的宽度比之前的宽度大，
